...
<script>
	...
	window.onload = function () {
		...
		bind_examples(4, 'y^\\\\prime=y^2+4x^2y^2', "y^2+4*x^2*y^2", "", 0);
		bind_examples(5, 'y^\\\\prime=x^3+2yx^3', "x^3+2*y*x^3", "", 0);
		?>
		$("#in-ch0").change(function () {
			$("#in-ode2").attr("readonly", true);
			$("#in-ode").attr("readonly", false);
		});
		$("#in-ch1").change(function () {
			$("#in-ode").attr("readonly", true);
			$("#in-ode2").attr("readonly", false);
		});
	}
