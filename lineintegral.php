...
<form name="exampleform" id="exampleform"
	...
	<fieldset class="vnitrni">
	  ...
	  ?>
		...
		<input type="radio" name="kind" value="1" id="ch-kind1" <?php if ($kind == 1) {
...
<input size="20" id="in-function"
	   ...
	   ?>">d<i>s</i>
		...
		<input type="radio" name="kind" id="ch-kind2" value="2" <?php if ($kind == 2) {
		echo "checked=\"checked\"";
	} ?> >
...
<input size="6"
	   ...
	   if ($fz == "") {
		   ...
		   echo rawurldecode($fz);
	   }
	   ?>">d<i>z</i>
<span
		class="submit_comment"><br>  &nbsp;<?php echo __("Leave the last field empty for integral in 2D plane."); ?></span>
</span>
	</fieldset>
	...
	<fieldset class="vnitrni">
		...
		<div style="display:none;">
			<div class="vnitrni2">
		  <?php $dimension = $_REQUEST["dimension"];
		  if ($dimension != "3") {
			  $dimension = 2;
		  } ?>
				<input type="radio" id="ch-dim2" name="dimension" value="2" <?php if ($dimension == "2") {
			echo "checked=\"checked\"";
		} ?>
							 onclick="document.getElementById('z-coord').style.display='none';"
				> <?php echo __("line integral in 2D"); ?>
				<br>
				<input type="radio" id="ch-dim3" name="dimension" value="3" <?php if ($dimension == "3") {
			echo "checked=\"checked\"";
		} ?>
   ...
   <span class="submit_comment"><?php echo __("Leave the last field empty for integral in 2D plane."); ?></span>
		...
		<fieldset class="vnitrni2">
		  ...
		  <?php echo __("Parameter bounds"); ?>
			...
			tmin=<input size="10" id="in-tmin" name="tmin" value="<?php $tmin = $_REQUEST["tmin"];
		if ($tmin == "") {
			echo '0';
		} else {
			echo rawurldecode($tmin);
		} ?>">
			<br>
			tmax=<input size="10" id="in-tmax" name="tmax" value="<?php $tmin = $_REQUEST["tmax"];
		if ($tmax == "") {
			echo 'pi';
		} else {
			echo rawurldecode($tmax);
		} ?>">
			<br>
			<span id="preview_curve"><input value="<?php echo(__("Preview")); ?>" onclick="preview_curve()" type="button"
																			class="tlacitko"></span>
		</fieldset>
	</fieldset>
